...
with Session(engine) as session:
    ...
    from .dev_data import user_events
    for user, event in user_events.pairs:
        user_entity = session.get(UserEntity, user.id)
        event_entity = session.get(EventEntity, event.id)
...
with Session(engine) as session:
    ...
    for user, event in user_events.pairs:
        user_entity = session.get(UserEntity, user.id)
        event_entity = session.get(EventEntity, event.id)
        user_entity.events.append(event_entity)
    session.commit()
